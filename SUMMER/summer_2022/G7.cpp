...
#define START -3.14159
#define END 0
#define STEP 0.1

//  y''' + y''sin(x) - y'cos(x) = cos(x)(2sin(2x) - 1) - 4sin(x)cos(2x) + 8sin(2x) - 1
//  -pi < x < 0
//  y(-pi) = 1
//  y'(-pi) = -1
//  y''(-pi) = -4
...
void f_km(double x, int i)
{
    ...
    dif_km[3][i] = - dif_rk[2][i] * sin(x) + dif_rk[1][i] * cos(x) + cos(x) * (2 * sin(2*x) - 1) - 4 * sin(x) * cos(2*x) + 8 * sin(2*x) - 1; 
}
double f_for_k(double y)
{
    double k[3];
...
void f_rk(double x, int i)
{
    ...
    dif_rk[3][i] = - dif_rk[2][i] * sin(x) + dif_rk[1][i] * cos(x) + cos(x) * (2 * sin(2*x) - 1) - 4 * sin(x) * cos(2*x) + 8 * sin(2*x) - 1; 
}
void calculate(void (func)(double, int), int n) {
    double x_i;
    for (int i = 0; i < n; i++) {
